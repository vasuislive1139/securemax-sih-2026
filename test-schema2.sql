-- ============================================================
-- SecureMax SIH26125 — Final Reconciled Schema Migration
...
-- 1. Strict Backend Isolation (Service Role bypasses RLS)
...
-- 2. Scoped Authenticated Access for Client Sessions (Enforces DB-Level IDOR Protection)
CREATE POLICY "Users can read own assignments" ON asset_assignments FOR SELECT USING (
  auth.role() = 'authenticated' AND (
    auth.uid() = user_id 
    OR current_setting('request.jwt.claims', true)::jsonb->>'userId' = user_id::text
  )
);

CREATE POLICY "Users can read own permissions" ON asset_permissions FOR SELECT USING (
  auth.role() = 'authenticated' AND assignment_id IN (
    SELECT id FROM asset_assignments WHERE (
      auth.uid() = user_id 
      OR current_setting('request.jwt.claims', true)::jsonb->>'userId' = user_id::text
    )
  )
